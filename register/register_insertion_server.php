...
$calc = hash_hmac('sha256', 'register_insertion_server.php', $_SESSION['second_token']);
		...
		else
		{
			...
			if ($stmt = $con->prepare('INSERT INTO accounts (username, pass, email, phone_no, admin, activation_code) VALUES (?, ?, ?, ?, ?, ?)')) 
			{
					...
					if  ($stmt = $con->prepare('INSERT INTO security_questions VALUES (?, ?, ?, ?, ?, ?, ?)'))
					{
						...
						$activate_link = 'localhost/ComputerSecurity/register/activate_acc_server.php?email=' . $NEW_EMAIL . '&code=' . $uniqid;
